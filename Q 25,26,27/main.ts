console.log("\n Q:25 of day 9");
let alien_colour = "Purple";
if (alien_colour == "Purple"){
    console.log("You just earn 5points");
}
if (alien_colour == "white"){
    console.log("alien colour is white");
}
console.log("\n Q:26 of day 9");
let color_alien = "green"
if (color_alien == "green") {
    console.log("you just earn 5 points");
} else{
     console.log("you justsss earned 10 points");

}
if (color_alien == "black") {
    console.log("you just earn 5 points");
} else{
     console.log("you just earned 10 points");

}
//alien is yellow
console.log("\n Q:27 of day 9");
console.log("\n alien is yellow");
let aliencolor = "yellow";
if (aliencolor == "yellow"){
    console.log("you earn 5 points");
} else if(aliencolor == "red"){
    console.log("you just earned 10 points");
} else if(aliencolor == "blue"){
    console.log("You jusr earn 15 points");
}
//alien is blue
console.log("\n alien is blue")
let aliencolor2 = "blue";
if (aliencolor2 == "yellow"){
    console.log("you earn 5 points");
} else if(aliencolor2 == "red"){
    console.log("you just earned 10 points");
} else if(aliencolor2 == "blue"){
    console.log("You jusr earn 15 points");
}
//alien is red
console.log("\n alien is red");
let aliencolor3 = "red";
if (aliencolor3 == "yellow"){
    console.log("you earn 5 points");
} else if(aliencolor3 == "red"){
    console.log("you just earned 10 points");
} else if(aliencolor3 == "blue"){
    console.log("You jusr earn 15 points");
}